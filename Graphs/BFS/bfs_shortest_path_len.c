...
void bfs_visit(struct graph *g, size_t start, struct queue *q, int *visited, ssize_t *p) {
    p[start] = -1;
    enqueue(q, start);
    visited[start] = 1;
    while (!is_empty(q)) {
...
void bfs(struct graph *g, ssize_t *p, int *visited) {
...
int main() {
    ...
    printf("Shortest path lengths from source 0:\n");
    for (size_t i = 0; i < n; i++) {
        if (visited[i] == 0 && i != 0) { 
            printf("Node %zu is unreachable\n", i);
        } else {
            printf("Node %zu: %d\n", i, visited[i] - 1); 
        }
    }
